...
const SettingsPage: React.FC = () => {
  ...
  const openReassignDialog = (agent: string) => {
    ...
    setReassignTo("");
    // Default to half the agent's open load (rounded up), capped at total open.
  ...
  const submitReassign = async () => {
    ...
    const sourceRow = overviewByAgent.find((r) => r.agent === reassignFrom);
    if (!sourceRow) return;
    // Only open (non-resolved) conversations are eligible. Pick the most recent N.
    const eligible = sourceRow.convos
      .filter((c) => c.status !== "resolved")
      .slice(0, Math.max(1, Math.min(reassignCount, sourceRow.open)));
