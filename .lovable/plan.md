...
- The widget is a self-contained vanilla-JS bundle served from the app's own origin (`/widget/v1/Kit TJ Services ClientHub-widget.js`), built by Vite as a separate library entry. No React on the customer's site — keeps payload small and avoids version conflicts.
- Customer install snippet:
  ```html
  <script async src="https://<app-domain>/widget/v1/Kit TJ Services ClientHub-widget.js"
...
- Subsequent visits: `localStorage` key `Kit TJ Services ClientHub.widget.session` holds `{ conversationId, visitorId, name, email }` so the same thread resumes.
...
- `vite.widget.config.ts` — separate library build outputting `public/widget/v1/Kit TJ Services ClientHub-widget.js`.
...
- New `CookieConsent` component shown on first visit (any route, public or authed). Stores choice in `localStorage` key `Kit TJ Services ClientHub.consent.v1` = `{ essential: true, analytics: bool, acceptedAt }`.
- "Accept all" / "Essential only" / "Customize" buttons.
- The widget script reads the same key when embedded on `Kit TJ Services ClientHub.dev` itself; on third-party sites the widget shows its own mini-consent line in the intake form (already covered above).
- Re-open via footer "Cookie preferences" link.

### Data subject rights (in `/settings`)

- New "Privacy & data" card with:
  - **Export my data** — calls a new `exportMyData` callable that returns a JSON dump of the user's profile + audit entries authored by them; downloaded as `Kit TJ Services ClientHub-export-<uid>.json`.
...
- Contents (responsive: 1 column mobile, 4 columns ≥md):
  - Brand block: Kit TJ Services ClientHub mark + tagline + © year.
...
- Bottom strip: short notice — "We use essential cookies to run Kit TJ Services ClientHub and optional analytics cookies with your consent. See our Cookie Policy."
...
- Update `mem://index.md` with: footer present on every page; `/legal/*` is public; widget bundle path is stable at `/widget/v1/Kit TJ Services ClientHub-widget.js`.
